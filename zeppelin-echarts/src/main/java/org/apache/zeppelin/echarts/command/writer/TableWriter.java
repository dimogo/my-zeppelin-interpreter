 ...
 */
public class TableWriter extends Writer<String, InterpreterResult> {

	/**
	 * 紧跟命令后的table,附加在前一条命令的结果后一起输出
	 */
	...
	public InterpreterResult execute(String input, PropertyGetter propertyGetter, InterpreterContext interpreterContext) {
		return new InterpreterResult(InterpreterResult.Code.SUCCESS, InterpreterResult.Type.TABLE, OutputCombiner.combine(input, this.table));
	}
}
